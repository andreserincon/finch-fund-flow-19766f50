...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const memberBalances = members.map((m: any) => {
      // Calculate total fees owed up to monthEndStr
      let totalFeesOwed = 0;
      allMonthlyFees.forEach((fee: any) => {
        const feeMonth = fee.year_month;
        // Only count fees from join date to monthEndStr
        if (feeMonth >= m.join_date.substring(0, 7) + '-01' && feeMonth <= monthEndStr) {
          const memberFeeType = getMemberFeeTypeForMonth(m.id, feeMonth);
      ...
      const balance = memberPayments - totalFeesOwed;
